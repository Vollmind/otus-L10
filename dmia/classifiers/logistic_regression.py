...
class LogisticRegression:
    ...
    def train(self, X, y, learning_rate=1.0, reg=1e-5, num_iters=100,
        ...
        for it in range(num_iters):
            ...
            choosen_num = np.random.choice(num_train, batch_size)
            X_batch = X[choosen_num]
            y_batch = y[choosen_num]
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        ###########################################################################
        h = 1 / (1 + np.exp(-np.sum(X.multiply(self.w), axis=1)))
        y_proba = np.hstack((h, -h+1))
    ...
    def predict(self, X):
        ...
        y_proba_transposed = np.transpose(y_proba)
        y_pred = np.choose(y_proba[:, 0] > y_proba[:, 1], [0, 1])
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        # result = exp(np.matmul(X_batch.toarray(), self.w))
        h = 1 / (1 + np.exp(-np.sum(X_batch.multiply(self.w).toarray(), axis=1)))
        dw = (y_batch - h) * X_batch / y_batch.size
        loss = np.sum(y_batch - h) / y_batch.size
